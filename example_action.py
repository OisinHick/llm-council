...
async def perform_action(task: str, execute: bool = True, stream: bool = False):
    ...
    payload = {"request": task, "execute": execute}

    print(f"\n📋 Council Task: {task}\n")
    print(
        f"{'Execute' if execute else 'Plan only'} | {'Streaming' if stream else 'Full response'}\n"
    )
            ...
            if line.startswith("data: "):
                try:
                    ...
                    logging.getLogger(__name__).debug(
                        "Skipping non-JSON SSE line: %s", line[:200]
                    )
...
def print_event(event: dict):
    ...
    if event_type == "stage1_start":
        ...
        if plan.get("success"):
            ...
            print("✓ Stage 4 complete: Action plan ready")
            ...
            print(
                f"✗ Failed to generate action plan: {plan.get('error', 'Unknown error')}"
            )
